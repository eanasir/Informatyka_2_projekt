...
{ 
    ...
    if (bgiemu_handle_redraw || visual_page != active_page) { 
        TextOut(hdc[1], x, y, (LPCWSTR)str, strlen(str));
    }
    if (visual_page == active_page) { 
        TextOut(hdc[0], x, y, (LPCWSTR)str, strlen(str));
    } 
}
...
int textheight(const char* str)
{
    SIZE ss;
    select_font();
    GetTextExtentPoint32(hdc[0], (LPCWSTR)str, strlen(str), &ss);
    return ss.cy;
}
			   ...
			   int size_width, int size_height)
{
	...
	wcApp.lpszClassName = (LPCWSTR)"BGIlibrary";
	wcApp.hInstance = 0;
	...
	hWnd = CreateWindow((LPCWSTR)"BGIlibrary", (LPCWSTR)"Windows BGI",
		WS_OVERLAPPEDWINDOW,
